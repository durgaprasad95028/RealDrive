...
import { EXOTIC_SUPERCAR_DATABASE_PART_1, DeepVehicleEngineeringRecord } from '../../database/seed_data/ExoticSupercarDatabasePart1.js';
import { UsedCarMarketplaceService, USED_CAR_MARKETPLACE_DATABASE } from '../../database/seed_data/UsedCarMarketplaceHistoricalDataset.js';
...
export class ShowroomInventoryService {
  ...
  public getAvailableShowroomVehicles(): ShowroomVehicleDisplay[] {
    return EXOTIC_SUPERCAR_DATABASE_PART_1.map((veh: DeepVehicleEngineeringRecord) => {
      const msrp = veh.peakEngineHorsepower * 220 + 45000;
      return {
        vehicleId: veh.modelCode,
        brand: veh.badgeManufacturer,
        name: veh.officialName,
        category: veh.homologationClass,
        baseMSRPUSD: msrp,
        horsepowerHp: veh.peakEngineHorsepower,
        torqueNm: veh.peakEngineTorqueNm,
        zeroTo100Sec: Number((3200 / veh.peakEngineHorsepower).toFixed(2)),
        topSpeedKph: Math.round(Math.min(390, 240 + veh.peakEngineHorsepower * 0.18)),
        availableTrims: [
